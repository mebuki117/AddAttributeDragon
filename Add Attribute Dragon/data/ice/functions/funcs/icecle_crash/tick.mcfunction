...
## spawn armor_stand
execute align xz at @a[tag=aad_i_icecle] run summon minecraft:armor_stand ~0.5 ~130 ~0.5 {Invisible:1b,Rotation:[0.0f,0.0f],Team:"aad_i_icecle"}
## remove a chance tag
execute as @a[tag=aad_i_icecle] run tag @s remove aad_i_icecle


### i hate collisionrule
execute as @e[type=minecraft:armor_stand,team=aad_i_icecle] at @s if entity @e[type=minecraft:ender_dragon,distance=..15] run fill ~-1 ~ ~-1 ~1 ~10 ~1 minecraft:air replace minecraft:packed_ice
execute as @e[type=minecraft:armor_stand,team=aad_i_icecle] at @s if entity @e[type=minecraft:ender_dragon,distance=..15] run kill @s
...
### fall down stopper
execute as @e[type=minecraft:armor_stand,team=aad_i_icecle] at @s unless block ~ ~-1 ~ minecraft:air run schedule function ice:funcs/icecle_crash/stopper 3t append


### utils
## damage
execute at @e[type=minecraft:armor_stand,team=aad_i_icecle,tag=!aad_i_death_c] run effect give @e[distance=..3] minecraft:instant_damage 1 1 true
